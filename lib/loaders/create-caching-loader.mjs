...
export default function createCachingLoader(loader, cache) {
	let cachingLoader = function (name, callback) {
		...
		cachingLoader.loader(name, (data) => {
	...
	cachingLoader.getStatus = function(key) {
		if(cachingLoader.cache && key in cachingLoader.cache) {
			if(cachingLoader.cache[key] === undefined) {
				return false
			}
			return true
		}
	...
	cachingLoader.getValue = function(key) {
		if(cachingLoader.cache && key in cachingLoader.cache) {
			return cachingLoader.cache[key]
		}
		if(loader.getValue) {
