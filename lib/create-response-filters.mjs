...
export default function createResponseFilters(webhandle) {
	function responseFiltersRouter(req, res, next) {
		 ...
		 * If a function is passed, it is turned into a transform stream. If the option `paused` is `false`, the
		 * stream will receive information as it becomes available instead of all at once at the end.
		 */
		res.addFilter = function (filter, {paused = true} = {}) {
			if (isStream(filter)) {
		...
		res.render = function (name, data, callback) {
			...
			try {
				let tailStream
				if (!!callback) {
					tailStream = new PausingTransform()
					// We need to make sure we don't flush away the information in the final stream
					tailStream._flush = function (callback) {
						if (callback) {
							callback()
						}
					}
				...
				return res.internalRender(name, data, (err, content) => {
					if (callback) {
						// Let's make sure everything is in run mode
						responseFilters.forEach(stream => {
							if (stream instanceof PausingTransform) {
								stream.run()
							}
						...
						if (tailStream instanceof PausingTransform) {
							tailStream.run()
						}
						...
						// We'll run everything to be sure it flushes out.
						responseFilters.forEach(stream => {
							if (stream instanceof PausingTransform) {
								stream.run()
							}
